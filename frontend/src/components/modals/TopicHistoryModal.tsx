// frontend/src/components/modals/TopicHistoryModal.tsx
/**
 * Bottom‑sheet modal that displays the full timeline of a topic's evolution.
 * Each history point can be expanded to show summary, key insights, and a development note.
...
// Structure of a history point as returned by the backend API.
...
// Visual configuration for each history point type.
...
const TopicHistoryModal: React.FC<Props> = ({
  ...
  // PanResponder enables swipe‑down to close without interfering with ScrollView.
  ...
  const fetchHistory = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/topics/${topicId}/history`);
          ...
          <View style={styles.nodeContent}>
            ...
            >
              <View style={styles.nodeHeader}>
                ...
                {/* Show a badge if this history point's metadata was regenerated by the LLM. */}
              ...
              {/* A short note summarising how the story changed (only for major updates). */}
  ...
  return (
    <Modal visible={visible} transparent animationType="none" onRequestClose={handleClose}>
      ...
      >
        {/* PanResponder is attached only to the drag zone so the ScrollView remains interactive. */}
